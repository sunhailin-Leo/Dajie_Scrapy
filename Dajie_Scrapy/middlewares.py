...
class DaJieScrapyUserAgentMiddleware(UserAgentMiddleware):
    """
    ...
    def process_request(self, request, spider):
        request.headers[':authority'] = 'so.dajie.com'
        request.headers[':method'] = 'GET'
        request.headers[':scheme'] = 'https'
        request.headers['accept'] = '*/*'
        # request.headers['accept-encoding'] = 'gzip, deflate, br'
        request.headers['accept-language'] = 'zh-CN,zh;q=0.9'
        request.headers['Cookie'] = settings['DAJIE_COOKIE']
        request.headers['Referer'] = "https://so.dajie.com/job/search?keyword=" + up.quote(settings['REFERER_NAME']) +\
                                     "&from=job&clicktype=blank"
